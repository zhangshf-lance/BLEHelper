...
- **WiFi 调试**：分为 HOSTAP 与 STATION 两个区域，支持 HOSTAP 配置/启动/停止/状态查询，以及 STATION 扫描/连接/断开/状态查询。
...
如果希望外部扫描列表里的蓝牙名称也匹配界面填写的“系统蓝牙名称”，请使用管理员方式启动：
...
```text
dist\EmbeddedDebugAssistant.exe
```
...
> 串口和 WiFi 功能不需要第三方库。BLE 主设备需要 `bleak`，BLE 从设备使用 `bless`。当前 requirements 显式固定了 Windows 可用组合，并使用 `--no-deps` 避免 `bless` 的依赖声明把 BLE 包降级。电脑蓝牙适配器仍必须支持 BLE 广播/GATT Server。

## 使用建议

1. 先打开“串口通信”，点击“刷新”查看 COM 口。蓝牙经典 SPP 配对后通常会出现在这里。
2. 打开“BLE 主设备”，点击“扫描”，选择设备后“连接”，再选择特征进行读写或订阅通知。
...
8. BLE 主设备、BLE 从设备、串口通信、WiFi 输出、运行日志都提供独立清除按钮，可分别清空发送输入、接收窗口或日志内容。
9. BLE 主设备、BLE 从设备、串口通信都支持独立定时循环发送，间隔单位为毫秒；循环发送会沿用当前发送内容和 HEX/字符串设置。
10. BLE 主设备连接后会强制按未缓存方式发现 GATT 服务；若特征仍不完整，可点击“刷新特征”重新枚举。
11. WiFi HOSTAP 使用 Windows `netsh wlan hostednetwork` 能力，部分 Windows 11 无线网卡/驱动可能不再支持该模式；STATION 连接已有配置时可不填密码，填写密码时会自动创建当前用户 WiFi 配置。
12. WiFi 输出框只显示当前选中热点的详细信息，包括认证、加密、最佳信号、BSSID、无线类型、频道和速率等可从 Windows 扫描结果中读取到的字段。
13. 串口“多条字符串顺序发送”可保存多条字符串、注释、启用状态和每条发送后的延时；点击“顺序发送”会按表格从上到下发送启用项，配置保存在当前用户的 AppData 目录。
...
run_admin.bat                  管理员方式启动脚本
run_exe_admin.bat              管理员方式启动打包 EXE
...
ble_assistant/wifi_win.py      Windows WiFi HOSTAP/STATION 后端
pysetupdi.py                   bless 在 Windows 下需要的蓝牙适配器枚举兼容层
```

## 注意

Windows 中 BLE “从设备/GATT Server” 比 BLE 主设备更受系统版本、蓝牙驱动和 Python BLE 库影响。如果目标是调试蓝牙串口模块，优先使用系统配对后生成的 SPP COM 口；如果目标是调试 BLE 外设，优先使用“BLE 主设备”页。
